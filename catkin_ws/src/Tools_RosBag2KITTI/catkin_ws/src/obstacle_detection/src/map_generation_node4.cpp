...
MapGenerationNode::MapGenerationNode():	lidar_index(0), camera_index(0), camera_captured(false), it(nh), init_camera_time(false), init_lidar_time(false)
{
	...
	sub_camera = nh.subscribe("/flir_4", 1000, &MapGenerationNode::cameraCallback, this);
	sub_lidar = nh.subscribe("/livox_4", 1000, &MapGenerationNode::lidarCallback, this);
    ros::spin();
}
...
void MapGenerationNode::lidarCallback(const sensor_msgs::PointCloud2::ConstPtr& lidar)
{
	...
	char s[200];
	sprintf(s, "/home/kaai/catkin_ws/src/Tools_RosBag2KITTI/catkin_ws/output/K6/3/a4/%06lld.pcd", lidar_index); 
	++lidar_index;
...
void MapGenerationNode::cameraCallback(const sensor_msgs::ImageConstPtr& camera)
{
	...
	char s[200];
	sprintf(s, "/home/kaai/catkin_ws/src/Tools_RosBag2KITTI/catkin_ws/output/K6/3/flir4/%06lld.png", camera_index); 
    ++camera_index;
...
{
    ros::init(argc, argv, "map_generation4");
