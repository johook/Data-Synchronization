...
#------------------------------------------------------------------------------
    def callback4(self, msg):
        self.r0 = np.array([msg.point.x, msg.point.y, msg.point.z])

    def callback5(self, msg):
        self.r1 = np.array([msg.point.x, msg.point.y, msg.point.z])
#------------------------------------------------------------------------------
...
def get_roll(v1,v2) :
    t1 = np.dot(v1,v2)
    cos =t1/(np.linalg.norm(v1)*np.linalg.norm(v2)) #linalg()=벡터의 크기
...
#-------------------eye point----------------------------------------------------
        e0 = tracker.realsense2world(tracker.r0) # eye_point
        e1 = tracker.realsense2world(tracker.r1)  # eye_point 
#--------------------------------------------------------------------------------        
...
#-----------------------TODO : get yaw pitch roll , and matmul with v_eye---------------------------------------
        ...
        e0 =tuple(e0)
        e1 =tuple(e1)
        v_eye = tuple(test_gp) #tracker.gp test_gp
        #print("1 : ", v_eye)
        v_eye = q.qv_mult(q_orig,v_eye)
        #print('e1 ', eye_driver_right, eye_driver_right,'\n')
        ...
        marker_eye = make_marker(v_eye * 999999 + eye_center, eye_center, [1, 0, 0])
        marker_eye_right = make_marker(v_eye*10, e0, [0.5, 0.5, 0])
        marker_eye_left = make_marker(v_eye*10, e1, [1, 1, 0])
