...
class point_pub:
    def __init__(self, name, x, y, z):
        ...
        self.pointstamp.header.frame_id = "/livox_frame" # change velodyne to camera_link
        self.pub.publish(self.pointstamp)

#roll구하기
def get_roll(v1,v2) :
    t1 = np.dot(v1,v2) 
    cos =t1/(np.linalg.norm(v1)*np.linalg.norm(v2)) #곱linalg()=벡터의 크기
...
if __name__ == "__main__":
    ...
    tracker = Tracker(np.array([0.5,0,0]))  # real distance for realsense cam #이해x
...
#------------------추가-----------------
    ...
    pub_raw_eye = rospy.Publisher('/vector_eye_raw', Marker, queue_size=10)
    ...
    if TEST_EYE:
        ...
        for i in range(CALIB):
           ...
           # print("cline_1" + str(cline_1) + '\n')
            c_normal = np.cross(cline_0, cline_1) #외적
            if c_normal[0] < 0:
                c_normal = - c_normal #양수로 바꿔준다?->절대값 개념
