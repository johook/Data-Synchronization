...
def make_marker(pt1, pt2, color, namespace):
    marker = Marker()
...
class Tracker:
    ...
    def callback0(self, msg):
        self.v0 = np.array([msg.point.x, msg.point.y, msg.point.z]) #pink1

    def callback1(self, msg):
        self.v1 = np.array([msg.point.x, msg.point.y, msg.point.z]) #pink2

    def callback2(self, msg):
        self.v2 = np.array([msg.point.x, msg.point.y, msg.point.z]) #pink3

    def callback4(self, msg):
        self.r0 = np.array([msg.point.x, msg.point.y, msg.point.z]) #yellow1

    def callback5(self, msg):
        self.r1 = np.array([msg.point.x, msg.point.y, msg.point.z]) #yellow2
...
if __name__ == "__main__":
    ...
    if TEST_EYE:
        ...
        for i in range(CALIB):
            cline_0 = tracker.r0 - tracker.v0 #r0 yellow v0 pink
           # print("cline_0" + str(cline_0) + '\n')
            cline_1 = tracker.r0 - tracker.v1 #r0 yellow v1 pink
...
#------------------------------------------------------
    ...
    while not rospy.is_shutdown():
        ...
        line1 = tracker.v1 - tracker.r0
        
        r_line0 = tracker.r0 - tracker.v0
        ...
        marker_head = make_marker(v_head * 999999 + p_driver, p_driver, [0, 1, 0], 'head') #org  
        marker_normal = make_marker(normal * 999999 + p_driver, p_driver, [0, 0, 1], 'normal') #org
...
#---TODO : get yaw pitch roll , and matmul with v_eye---------------------------------------
        ...
        marker_raw_eye = make_marker(v_raw_eye * 999999 , p_driver, [1, 1, 0], 'raw_eye')
         
        marker_eye = make_marker(v_eye * 999999+p_driver , p_driver, [1, 0, 0], 'eye')
        marker_eye_right = make_marker(v_eye*10+e0, e0, [0.5, 0.5, 0], 'eye_right')
        marker_eye_left = make_marker(v_eye*10+e1, e1, [1, 1, 0], 'eye_left')
