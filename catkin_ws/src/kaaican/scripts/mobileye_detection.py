...
from kaaican.msg import Mobileye_det
...
def func_0x669(msg):
    ...
    ''' decorder '''
    value = int((msg.data[2]*16 + (int(msg.data[1])&0xF0)/16))
    ...
    value = int((msg.data[7]*16 + (int(msg.data[6])&0xF0)/16))
    if value &0x800== 0x800:
...
def func_detect(msg):
    ...
    data_mobileye_test.o_position_x[num] = ((int(msg.data[2]) & 0x0f) * (16 ** 2) + msg.data[1]) * 0.0625
    value = int((int(msg.data[4]) & 0x03) * (16 ** 2) + msg.data[3])
    ...
    value = int((int(msg.data[6]) & 0x0f) * (16 ** 2) + msg.data[5])
    if value &0x800== 0x800:
    ...
    data_mobileye_test.o_brake_light[num] = (int(msg.data[7]) & 0x08) // 8
    data_mobileye_test.o_type[num] = (int(msg.data[6]) & 0x70) // 16
    data_mobileye_test.o_status[num] = (int(msg.data[7]) & 0x07)
